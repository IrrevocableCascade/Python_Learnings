...
though this Collatz Conjecture was expressed in 1937, up to now no one could find a proof that it is really so for any
X or could not find a counterexample (i.e. number for which sequence did not end with 1 - either entering some bigger
loop or growing infinitely).
...
Answer should contain the same amount of results, each of them being the count of steps for getting Collatz Sequence to
 1."""

def collatz(filename):
	...
	for seed in seeds:
		results.append(do_sequence(seed))

	return results

def do_sequence(seed):
	sequence = []
	xnext = int(seed)
